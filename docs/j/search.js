...
function updateSearchResults_old(query, results) {
...
function updateSearchResults(query, results) {
    ...
    docsStr = results
        .map(
            (hit) => `
                      <div class="m-2 mb-8 flex flex-col md:flex-row w-full">
			...
			<div class="w-full p-2 pb-2 md:w-1/4">
			  <div class="flex flex-row">
			    <div class="px-2">
			      <a target="_blank" href="${hit.url}"><img class="mt-4 scale-200" src="../../i/pdf-large.svg"></a>
			    </div>
			    <div class="px-2">
    ...
    document.getElementById("query").innerHTML = "Search Terms: \"" + query + "\" " + `(${results.length} results)`;
    document.getElementById("search-cells").innerHTML = docsStr
...
function displayErrorMessage(message) {
    console.log("INSIDE displayErrorMessage: " + message);
    ...
    // document.querySelector(".search-error").classList.add("animate-pulse");
