...
DeptListIDs = {'Environment Department': 7, 'Finance Department': 8, 'Food, Civil Supplies and Consumer Protection Department': 9, 'General Administration Department': 10, 'Higher and Technical Education Department': 11, 'Home Department': 12, 'Housing Department': 13, 'Industries, Energy and Labour Department': 14, 'Information Technology Department': 15, 'Law and Judiciary Department': 16, 'Marathi Language Department': 17, 'Medical Education and Drugs Department': 18, 'Minorities Development Department': 19, 'Other Backward Bahujan Welfare Department': 20, 'Parliamentary Affairs Department': 21, 'Persons with Disabilities Welfare Department': 22, 'Planning Department': 23, 'Public Health Department': 24, 'Public Works Department': 25, 'Revenue and Forest Department': 26, 'Rural Development Department': 27, 'School Education and Sports Department': 28, 'Skill Development and Entrepreneurship Department': 29, 'Social Justice and Special Assistance Department': 30, 'Soil and Water Conservation Department': 31, 'Tourism and Cultural Affairs Department': 32, 'Tribal Development Department': 33, 'Urban Development Department': 34, 'Water Resources Department': 35, 'Water Supply and Sanitation Department': 36, 'Women and Child Development Department': 37, 'Agriculture, Dairy Development, Animal Husbandry and Fisheries Department': 5, 'Co-operation, Textiles and Marketing Department': 6}
...
def get_week_document_infos(gr_dir, year, wk_num):
    ...
    [i.__setitem__('date', datetime.datetime.strptime(i['date'], "%d-%m-%Y").date()) for i in doc_infos]
...
def write_search_index(output_dir, new_doc_infos):
    ...
    if docs_file.exists():
        old_search_dicts = json.loads(docs_file.read_text())
